	...
	if *exportPlain || *exportCloud {
		...
		*pcrIndexes = strings.TrimSpace(*pcrIndexes)
		...
		if *exportCloud && *output != "" {
			...
			log("[DEBUG] Disk Key : %X\n", diskKey)
			log("[DEBUG] Disk Key Digest : %X\n", digest256)
			log("[DEBUG] Encrypted Disk Key : %X\n", encryptedDiskKey)
			...
			log("[DEBUG] AttestVolumeKeyData : %X\n", encryptedVaultKey)
			log("[DEBUG] AttestVolumeKeyData (b64) : %s\n", base64.StdEncoding.EncodeToString(encryptedVaultKey))
			...
			log("[DEBUG] AttestVolumeKey : %X\n", volumeKey)
			log("[DEBUG] AttestVolumeKey (b64) : %s\n", base64.StdEncoding.EncodeToString(volumeKey))
